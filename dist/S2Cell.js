...
var S2Cell = /** @class */ (function () {
    ...
    S2Cell.fromFacePosLevel = function (face, pos, level) {
        return new S2Cell(S2CellId_1.S2CellId.fromFacePosLevel(face, new long_1.default(pos), level));
    };
    // Convenience methods.
    ...
    S2Cell.prototype.approxArea = function () {
        ...
        return flatArea
            .times(2)
            .dividedBy(decimal.Decimal.min(flatArea.times(S2_1.S2.M_1_PI), 1)
            .neg()
            .plus(1)
            .sqrt()
            .plus(1)).toNumber();
    ...
    S2Cell.MAX_ERROR = S2_1.S2.toDecimal(1.0).dividedBy(S2_1.S2.toDecimal(new long_1.default(1).shiftLeft(51).toString()));
    ...
    // roundoff errors
    S2Cell.POLE_MIN_LAT = decimal.Decimal.asin(S2_1.S2.toDecimal(1.0).dividedBy(3).sqrt()).minus(S2Cell.MAX_ERROR);
    return S2Cell;
}());
