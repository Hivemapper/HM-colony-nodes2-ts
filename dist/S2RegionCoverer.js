...
export class S2RegionCoverer {
    ...
    getInitialCandidates() {
        ...
        if (this.maxCells >= 4) {
            ...
            let level = Decimal.min(S2Projections.MIN_WIDTH.getMaxLevel(cap.angle().radians.times(2)), Decimal.min(this.maxLevel, S2CellId.MAX_LEVEL - 1)).toNumber();
...
S2RegionCoverer.FACE_CELLS = [0, 1, 2, 3, 4, 5].map(face => S2Cell.fromFacePosLevel(face, 0, 0));
class Candidate {
    // elements.
    toString() {
