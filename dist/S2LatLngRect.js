...
export class S2LatLngRect {
    ...
    intersects(cell) {
        ...
        for (let i = 0; i < 4; ++i) {
            const edgeLng = S1Interval.fromPointPair(cellLl[i].lngRadians, cellLl[(i + 1) & 3].lngRadians);
    ...
    static /*boolean*/ intersectsLatEdge(a, b, lat, lng) {
        ...
        const abTheta = S1Interval.fromPointPair(decimal.Decimal.atan2(a.dotProd(y), a.dotProd(x)), decimal.Decimal.atan2(b.dotProd(y), b.dotProd(x)));
        if (abTheta.contains(theta)) {
            // Check if the intersection point is also in the given "lng" interval.
            const isect = S2Point.add(S2Point.mul(x, cosTheta), S2Point.mul(y, sinTheta));
